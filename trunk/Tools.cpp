...
/// <summary>
/// Adding to last servers list new address
/// </summary>
void Tools::addToLastServers(lastServers* lServ)
{
	lastServ[lastUsedServer].ipAddress = lServ->ipAddress;
	lastServ[lastUsedServer].port = lServ->port;
	lastServ[lastUsedServer].time = lServ->time;
	lastServ[lastUsedServer].protocol = lServ->protocol;
	lastUsedServer++;
}

/// <summary>
/// Loading last servers from *.xml file
/// </summary>
bool Tools::loadFromXmlLastServersList()
{
	for(int i = 0; i < MAX_AMOUNT_OF_LAST_SERVERS; i++)
		ZeroMemory(&lastServ, sizeof(lastServ));

	xmlDocPtr doc = xmlParseFile(LAST_SERVERS_FILE);
	if(!doc)
		return false;
	xmlNodePtr root, p;
	root = xmlDocGetRootElement(doc);
	if(xmlStrcmp(root->name,(const xmlChar*)"LastServers") != 0)
	{
		xmlFreeDoc(doc);
		return false;
	}
	...
	while(p)
	{
		...
		{
			if(readXMLString(p, "IP", strVal))
				strcpy(lastServ[cID].ipAddress, strVal.c_str());
			if(readXMLString(p, "Port", strVal))
				lastServ[cID].port = atoi(strVal.c_str());
			if(readXMLString(p, "Time", strVal))
				lastServ[cID].time = atoi(strVal.c_str());
			if(readXMLString(p, "Protocol", strVal))
				strcpy(lastServ[cID].protocol, strVal.c_str());
			cID++;
		}
		p = p->next;
	}
